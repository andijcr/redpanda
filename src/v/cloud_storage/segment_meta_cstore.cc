...
public:
    ...
    bool can_be_inserted(const segment_meta& m) const {
        ...
        // implementation, takes as parameters adapters to be able to operate on
        // both _write_buffer and counter_col_t search m.base_offset. either
        ...
        constexpr static auto impl_can_be_inserted =
          [](
            auto& sm,
            ...
            auto prev_op) {
              ...
              if (
                ...
                && dereference_op(candidate) == sm.base_offset) {
                  ...
                  return committed_offset_dereference_op(candidate)
                         <= sm.committed_offset;
              }
              ...
              // and it's not equal)
              if (
                dereference_op(prev_it) < sm.base_offset
                && committed_offset_dereference_op(prev_it)
                     >= sm.committed_offset) {
        ...
        // check _write_buffer first, then cstore. use lambda to adapt both to
        // the implementation
        return likely(impl_can_be_inserted(
                 m,
                 _write_buffer,
                 [](auto& it) { return it->second.base_offset; },
                 [](auto& it) { return it->second.committed_offset; },
                 [](auto& it) { return std::prev(it); }))
               && impl_can_be_inserted(
                 m,
                 base_col,
                 [](auto& it) { return *it; },
                 [&](auto& it) { return *committed_col.at_index(it.index()); },
                 [&](auto& it) {
    ...
    auto covered_range(const segment_meta& sm) const -> std::pair<
      ...
      std::unique_ptr<segment_meta_materializing_iterator::impl>> {
        ...
        for (; (!it->equal(*it_end))
               && it->dereference().base_offset >= sm.base_offset
               && it->dereference().committed_offset <= sm.committed_offset;
             it->increment()) {
        }
